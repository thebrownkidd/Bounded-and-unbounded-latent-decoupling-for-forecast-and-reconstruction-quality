...
Tests AE+DMD vs Resid+GRU+DMD across observation noise levels.

Outputs: /kaggle/working/noise_sweep.json
"""
...
def run_one(cfg, seed):
    ...
    # AE+DMD
    SeedAll(seed)
    ae = AE(n_obs, k, h).to(DEVICE); train_ae(ae, Xt); ae.eval()
    with torch.no_grad(): Z = ae.encode(Xt).cpu().numpy()
    A = fit_dmd(Z)
    def _r(tn, m=ae):
        with torch.no_grad():
            return m(torch.tensor(tn, dtype=torch.float32).to(DEVICE)).cpu().numpy()
    def _f(m=ae, A_=A):
        with torch.no_grad():
            z0 = m.encode(torch.tensor(test_n[:1], dtype=torch.float32).to(DEVICE)).cpu().numpy().ravel()
    ...
    # Resid+GRU
    SeedAll(seed)
    r2 = ResidualGRU(n_obs, j, k, h=h).to(DEVICE)
    ...
    with torch.no_grad(): B2 = r2.f(dC2).cpu().numpy()
    A2 = fit_dmd(B2)
    def _r2(tn, m=r2):
        with torch.no_grad():
    ...
    def _f2(m=r2, A_=A2):
        with torch.no_grad():
            ...
            Cc = m.carrier(torch.tensor(test_n[:1], dtype=torch.float32).to(DEVICE))
            b0 = m.f(Cc - Cp).cpu().numpy().ravel()
...
if __name__ == "__main__":
    ...
    with open(f"{OUT}/noise_sweep.json", "w", encoding="utf-8") as f:
