...
Analytical DMD rollout + learned correction networks.

Outputs: /kaggle/working/nonautoregressive.json
"""
...
class CumsumCorrection(nn.Module):
    ...
    def forward(self, deltas, C0):
        ...
        gated = deltas * gate.unsqueeze(0).unsqueeze(0)
        cumulative = torch.cumsum(gated, dim=1)
...
class Conv1dCorrection(nn.Module):
    ...
    def forward(self, deltas, C0):
        x = deltas.transpose(1, 2)
        x = torch.nn.functional.pad(x, (self.pad, 0))
        correction = self.net(x).transpose(1, 2)
...
class MLPCorrection(nn.Module):
    ...
    def forward(self, deltas, C0):
        ...
        pos = torch.arange(1, H + 1, dtype=deltas.dtype,
                          device=deltas.device).unsqueeze(0) / H
        pos = pos.unsqueeze(-1).expand(B, H, 1)
        cum_delta = torch.cumsum(deltas, dim=1)
        C0_exp = C0.unsqueeze(1).expand(B, H, j)
        inp = torch.cat([cum_delta, C0_exp, pos], dim=-1)
...
def train_correction(correction_net, model, C_all, B_all, A_dmd, unroll=UNROLL):
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for i in range(0, N, BS):
            sl = idx[i:i + BS]
            starts = sl.numpy()
            C0 = C_all[starts].to(DEVICE)
            C_target = torch.stack([C_all[s + 1:s + 1 + unroll] for s in starts]).to(DEVICE)

            b0s = B_all[starts].cpu().numpy()
            deltas_list = []
            for bi in range(len(b0s)):
                b_traj = dmd_rollout_batch(A_dmd, b0s[bi], unroll)
                with torch.no_grad():
                    delta = model.m(torch.tensor(b_traj, dtype=torch.float32).to(DEVICE))
                deltas_list.append(delta)
            deltas = torch.stack(deltas_list)
...
def make_systems():
    ...
    systems["Lorenz-96"] = dict(
        train_n=trn, test_n=ten, gt_test=gt, mu=mu, sig=sig,
        n_obs=20, gt_dim=20, fcst_steps=500, j=16, k=8, h=64)
...
def run_one(cfg, seed):
    ...
    def _f_add(m=rm, A_=A_dmd):
        ...
        C_ = Cc_init.squeeze(0).cpu().numpy(); b = b0.copy()
        with torch.no_grad():
            fc[0] = m.dec(torch.tensor(C_, dtype=torch.float32).unsqueeze(0).to(DEVICE)).cpu().numpy().ravel()
        for t in range(1, fcst_steps):
            b = A_ @ b
            with torch.no_grad():
                dc = m.m(torch.tensor(b, dtype=torch.float32).unsqueeze(0).to(DEVICE)).cpu().numpy().ravel()
            C_ = C_ + dc
            with torch.no_grad():
                fc[t] = m.dec(torch.tensor(C_, dtype=torch.float32).unsqueeze(0).to(DEVICE)).cpu().numpy().ravel()
        return fc
    ...
    for corr_name, corr_cls in [("cumsum", CumsumCorrection),
        ...
        def _f_corr(m=rm, A_=A_dmd, c=corr):
            with torch.no_grad():
                ...
                Cc_init = m.carrier(torch.tensor(test_n[:1], dtype=torch.float32).to(DEVICE))
                b0 = m.f(Cc_init - Cp).cpu().numpy().ravel()
            b_traj = dmd_rollout_batch(A_, b0, fcst_steps)
            with torch.no_grad():
                deltas = m.m(torch.tensor(b_traj, dtype=torch.float32).to(DEVICE))
                C0 = Cc_init.squeeze(0)
                carriers = c(deltas.unsqueeze(0), C0.unsqueeze(0))
                fc = m.dec(carriers.squeeze(0)).cpu().numpy()
            return fc
...
if __name__ == "__main__":
    ...
    for sname, cfg in systems.items():
        ...
        with open(f"{OUT}/nonautoregressive.json", "w", encoding="utf-8") as f:
            json.dump(stats, f, indent=2)
        print(f"  [saved {sname} to nonautoregressive.json]")
