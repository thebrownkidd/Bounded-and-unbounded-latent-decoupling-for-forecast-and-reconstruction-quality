...
often degrades the other---an apparent Pareto frontier that practitioners
...
frozen, reconstruction quality is preserved by construction.  On five dynamical
systems spanning linear, oscillatory, and spatiotemporal dynamics, the decoupled
model reduces reconstruction error by up to 29$\times$ while matching or improving
forecast accuracy; an ablation confirms that GRU-gated accumulation is essential for
stable rollouts.
\end{abstract}
...
We demonstrate this on five dynamical systems: a residual decoupling architecture
consistently improves reconstruction while matching or improving forecasting
...
Figure~\ref{fig:frontier} shows this frontier on three systems.  Reconstruction
RMSE improves as $\phi \to 0$ and forecast RMSE improves as $\phi \to 1$, but
no single $\phi$ minimises both---the two objectives genuinely compete for the
same $k$ dimensions.  The frontier represents the best the coupled architecture
can do at any weight.
...
capacity fully.  On all three systems the decoupled model sits
...
\paragraph{Setup.}
We evaluate on five smooth dynamical systems: coupled harmonic oscillators
(6D linear), a linear 5D system with two frequencies, the Brusselator
chemical oscillator, the forced Duffing oscillator, and Lorenz-96
(20D spatiotemporal).
...
\begin{table}[t]
\caption{Results on five dynamical systems (mean$\pm$std over 5 seeds).
...
Coupled Harm. & 6D  & 4
  & \tpm{.090}{.004} & \tpm{.53}{.06}
  & \tpm{.003}{.000} & \tpm{4.5}{2.3}
  & \textbf{\tpm{.003}{.000}} & \tpm{.64}{.06} \\
Linear 5D     & 5D  & 4
  & \tpm{.005}{.002} & \tpm{.67}{.14}
...
Table~\ref{tab:main} reports all five systems.  On every system the decoupled
model reduces reconstruction error compared to AE+DMD.
On Linear 5D, Duffing, and Lorenz-96 the decoupled model strictly
Pareto-dominates (both metrics improve); on the 20D Lorenz-96 system,
reconstruction improves $1.25\times$ and forecast improves $1.06\times$,
demonstrating that the decoupling extends to higher-dimensional
spatiotemporal dynamics.  On Brusselator, reconstruction is matched within
noise ($0.002\pm0.001$ vs $0.002\pm0.001$) while forecast improves
$1.8\times$.  On Coupled Harmonic, the $29\times$ reconstruction improvement
comes with a 20\% forecast increase ($0.64$ vs $0.53$)---a tradeoff that the
coupled model cannot achieve at any $\phi$ (Figure~\ref{fig:frontier}).
...
rollout: Resid+add forecast RMSE exceeds AE+DMD on every system (by $2{-}21\times$;
on Lorenz-96 it diverges to RMSE $100$).  The GRU gate
(Eq.~\ref{eq:gru}) is essential for stable rollouts.
...
spaces consistently and substantially improves reconstruction while matching
or improving forecast quality, and the $\phi$-sweep frontier shows how much
further a future architecture might go.
