"""
Single-seed test: MLP accumulation for carrier update.
C_{t+1} = MLP([m(b_t); C_t]) instead of GRU(m(b_t), C_t).
DMD forecast head throughout. Only runs Resid+MLP.
"""
...
def train_mlp_accum(model, dC, Cc, Cn):
    for p in model.parameters(): p.requires_grad_(False)
    for p in model.accum.parameters(): p.requires_grad_(True)
    opt = torch.optim.Adam(model.accum.parameters(), lr=LR)
    for ep in range(1, EPOCHS+1):
        model.train(); idx = torch.randperm(len(Cc))
        for i in range(0, len(Cc), BS):
            sl = idx[i:i+BS]
            with torch.no_grad(): dh = model.m(model.f(dC[sl]))
            inp = torch.cat([dh, Cc[sl]], dim=1)
            loss = nn.functional.mse_loss(model.accum(inp), Cn[sl])
            opt.zero_grad(); loss.backward(); opt.step()
        ...
        def _f(m=model, A_=A_dmd):
            ...
            for t in range(1, fcst_steps):
                ...
                with torch.no_grad():
                    ...
                    inp = torch.cat([dh, C_.unsqueeze(0)], dim=1)
                    C_ = m.accum(inp).squeeze(0)
                    fc[t] = m.dec(C_.unsqueeze(0)).numpy().ravel()
            return fc
        rr, rf = eval_metrics(_r, _f, test_n, gt_test, mu, sig, gt_dim, fcst_steps)
        results[sname] = {"rmse_r": rr, "rmse_f": rf}
        print(f"r={rr:.4f}  f={rf:.3f}")

    with open(OUT / "mlp_accumulation.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\n→ {OUT / 'mlp_accumulation.json'}")
