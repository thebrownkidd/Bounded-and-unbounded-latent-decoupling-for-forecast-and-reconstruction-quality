...
Tests the decoupling principle across three forecast heads:
    DMD   (post-hoc linear)
    MLP   (learned nonlinear)
    ODE   (continuous-time, RK4-integrated neural ODE)

For each head, we run:
    Coupled   — standard AE with joint (1-φ)L_rec + φ L_fcst training
    Decoupled — carrier-residual architecture (teacher AE → f/m → GRU accumulation)

All methods use the same data, same architectures widths, and same evaluation.

Outputs:
    Paper/head_comparison.json — {system: {head: {coupled: ..., decoupled: ...}}}
"""
...
SEEDS = [0, 1, 2, 42, 123]
COUPLED_PHIS = [0.1, 0.5, 1.0]  # try a few, report best per system

parser = argparse.ArgumentParser()
parser.add_argument("--quick", action="store_true",
                    help="Smoke test: 5 epochs, 1 seed, 1 phi")
parser.add_argument("--system", nargs="+", default=None,
                    help="Run only these systems")
args = parser.parse_args()

if args.quick:
    EPOCHS = 5; SEEDS = [0]; COUPLED_PHIS = [0.5]
...
def fit_dmd(Z):
    """Post-hoc DMD: A = Y @ pinv(X)."""
    X, Y = Z[:-1].T, Z[1:].T
    return Y @ np.linalg.pinv(X)


class MLPHead(nn.Module):
    """b_{t+1} = MLP(b_t). Nonlinear discrete-time dynamics."""
    ...
    def forward(self, b):
        """(B, k) -> (B, k) one-step prediction."""
        return self.net(b)

    def step_numpy(self, b):
        """numpy in, numpy out — for rollout."""
        with torch.no_grad():
            return self.forward(
                torch.tensor(b, dtype=torch.float32).unsqueeze(0)
            ).squeeze(0).numpy()


class ODEFunc(nn.Module):
    """db/dt = f_θ(b). The vector field for the neural ODE."""
    def __init__(self, k, hidden=64):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(k, hidden), nn.ELU(),
...
class NeuralODEHead(nn.Module):
    """Neural ODE head: b(t+1) = b(t) + ∫_0^1 f_θ(b(s)) ds, solved by RK4.

    No external dependency (torchdiffeq not required). The RK4 solver with
    n_steps substeps integrates the learned vector field over [0, 1].
    """
    ...
    def forward(self, b):
        """(B, k) -> (B, k). RK4 integration of db/dt = f(b) over [0, 1]."""
        dt = self.dt
...
class CoupledAE(nn.Module):
    """Standard AE with a pluggable dynamics head, trained jointly."""
...
class ResidualModel(nn.Module):
    """Carrier-residual architecture (teacher AE + residual compressor)."""
...
def train_coupled(model, Xt, phi):
    """Joint training: (1-φ)L_rec + φ L_fcst, with gradient clipping."""
...
def train_resid(model, dC):
    """Phase 2: train f+m on carrier residuals."""
    for p in model.parameters(): p.requires_grad_(False)
...
def train_gru(model, dC, Cc, Cn):
    """Phase 3: train GRU for carrier accumulation."""
    for p in model.parameters(): p.requires_grad_(False)
...
def train_head_on_b(head, B_train):
    """Phase 2.5: train a dynamics head on the b trajectory.
    B_train: (T, k) tensor of residual codes."""
    opt = torch.optim.Adam(head.parameters(), lr=LR)
    bt, btp1 = B_train[:-1], B_train[1:]
    for ep in range(1, EPOCHS + 1):
        head.train(); idx = torch.randperm(len(bt))
...
def eval_metrics(recon_fn, fcst_fn, test_n, gt_test, mu, sig, gt_dim, fcst_steps):
    """Same eval protocol as run_multiseed.py."""
...
def run_one(sname, cfg, seed):
    """Run all heads (coupled + decoupled) for one system/seed."""
    ...
    # ------------------------------------------------------------------
    #  Coupled baselines: AE + {DMD, MLP, ODE}
    # ------------------------------------------------------------------
    for head_name, head_cls in [("MLP", MLPHead), ("ODE", NeuralODEHead)]:
        best_phi_res = None
        for phi in COUPLED_PHIS:
            SeedAll(seed)
            model = CoupledAE(n_obs, k, h, head_cls)
            train_coupled(model, Xt, phi); model.eval()

            def _r(tn, m=model):
                with torch.no_grad():
                    return m.decode(m.encode(
                        torch.tensor(tn, dtype=torch.float32))).numpy()

            def _f(m=model):
                with torch.no_grad():
                    z = m.encode(torch.tensor(test_n[:1], dtype=torch.float32)
                                ).squeeze(0).numpy()
    ...
    # AE+DMD (same as run_multiseed.py)
    SeedAll(seed)
    from collections import namedtuple
    ae_simple = type("AE", (nn.Module,), {
        "__init__": lambda self, n, k, h: (
            super(type(self), self).__init__(),
            setattr(self, "enc", nn.Sequential(
                nn.Linear(n, h), nn.ELU(), nn.Linear(h, h), nn.ELU(), nn.Linear(h, k))),
            setattr(self, "dec", nn.Sequential(
                nn.Linear(k, h), nn.ELU(), nn.Linear(h, h), nn.ELU(), nn.Linear(h, n))),
        )[-1],
        "encode": lambda self, x: self.enc(x),
        "decode": lambda self, x: self.dec(x),
        "forward": lambda self, x: self.decode(self.encode(x)),
    })(n_obs, k, h)
    # Train AE
    opt = torch.optim.Adam(ae_simple.parameters(), lr=LR)
    for ep in range(1, EPOCHS + 1):
        ae_simple.train(); idx = torch.randperm(len(Xt))
        for i in range(0, len(Xt), BS):
            loss = nn.functional.mse_loss(ae_simple(Xt[idx[i:i+BS]]), Xt[idx[i:i+BS]])
            opt.zero_grad(); loss.backward(); opt.step()
    ae_simple.eval()
    with torch.no_grad(): Z = ae_simple.encode(Xt).numpy()
    A_dmd = fit_dmd(Z)

    def _r_dmd(tn, m=ae_simple):
        with torch.no_grad():
            return m(torch.tensor(tn, dtype=torch.float32)).numpy()
    def _f_dmd(m=ae_simple, A_=A_dmd):
        with torch.no_grad():
    ...
    # ------------------------------------------------------------------
    #  Decoupled: Resid + {DMD, MLP, ODE} + GRU accumulation
    # ------------------------------------------------------------------

    # Phase 1+2+3: shared across all heads
    SeedAll(seed)
    ...
    def make_fcst_fn(m, step_fn):
        """Build a forecast function for a given b-space stepping function."""
        def _f():
            with torch.no_grad():
    ...
    # Resid+MLP
    SeedAll(seed + 1000)  # different seed for head, same teacher
    mlp_head = MLPHead(k)
...
if __name__ == "__main__":
    ...
    if args.system:
        systems = {s: systems[s] for s in args.system if s in systems}

    t0 = time.time()
    raw_results = {}  # {system: {method: [{rmse_r, rmse_f}, ...]}}
    ...
    # ── Save ──
    with open(OUT / "head_comparison.json", "w") as f:
